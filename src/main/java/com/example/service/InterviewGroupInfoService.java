...
public class InterviewGroupInfoService implements IInterviewGroupInfoService {
    ...
    @Override
    public PageInfo<InterviewGroupInfo> findGroupAll(String keyword, Integer pageSize, Integer pageNum) {
        System.out.println("分页请求："+keyword);
        PageHelper.startPage(pageNum, pageSize);
        if(!StrUtil.isNotEmpty(keyword)){
            keyword="";
        }
        List<InterviewGroupInfo> groups=interviewGroupInfoMapper.findGroupAll(keyword);
